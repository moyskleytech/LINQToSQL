...
namespace MoyskleyTech.LINQToSQL.Data
{
    public class ContextIndependantDatabase
    {
        ...
        public static R ConvertDatabaseObjectToCLRObject<R>(object value)
        {
            value = ConvertDatabaseObjectToCLRObject(value , typeof(R));
            return ( R ) value;
        }
        public static object ConvertDatabaseObjectToCLRObject(object value , Type outputType)
        {
            return ConvertDbToObject(value , outputType);
        }
        private static object ConvertDbToObject(object value , Type fieldType)
        {
            var isNullable = fieldType.Is(typeof(Nullable<>));
            if ( isNullable )
            {
        ...
        private static object ConvertDbToObject(object value , Type fieldType , object df)
        {
            if ( value == DBNull.Value || value == null )
                return df;
            try
            {
                var convertMethod = typeof(Convert).GetMethod("To" + fieldType.Name, new Type[] { value.GetType() , typeof(IFormatProvider)});
                if ( convertMethod != null )
                    value = convertMethod.Invoke(null , new object[ ] { value , new CultureInfo("en-US") });

                var convertMethod2 = typeof(Convert).GetMethod("To" + fieldType.Name, new Type[] { value.GetType() });
                if ( convertMethod2 != null )
                    value = convertMethod2.Invoke(null , new object[ ] { value });
            }
            catch
            {
                value = df;
