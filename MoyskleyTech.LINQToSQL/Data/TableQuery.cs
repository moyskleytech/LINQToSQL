    ...
    public class TableQuery<T> : BaseTableQuery, IEnumerable<T>
    {
            ...
            Expression<Func<TInner , TKey>> innerKeySelector)
        {
            ...
            if ( inner._where != null )
                q.AddWhere(Expression.Block(Expression.Constant(new MappingContext(inner.Table , ParentContext)) , inner._where));
        ...
        public TableQuery<T> UpdateAll<R>(Expression<Func<T , R>> expr , R value)
        {
            if ( _limit != null )
            {
                //SQLite provides a limit to deletions so this would be possible to implement in the future
                //You would need to take care that the correct order was being applied.
                throw new NotSupportedException("Cannot update if a limit has been specified");
            }
            if ( _offset != null )
            {
                throw new NotSupportedException("Cannot update if an offset has been specified");
            }
            ...
            var cmdText = "update " + Table.TableSource + " set ";
            ...
            var attr = memberExpression.Member.GetCustomAttribute<DataColumnAttribute>();
            cmdText += attr.Name + " = @updateValue";
            ...
            var command = Connection.CreateCommand(cmdText, args.ToArray());
            command.Parameters.AddWithValue("@updateValue" , value);
